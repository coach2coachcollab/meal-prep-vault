...
import { ChefHat, Target, Utensils, Eye, EyeOff, Zap } from "lucide-react";
import { useSeo } from "@/hooks/useSeo";
import { enterDemo } from "@/hooks/useAuth";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (error) throw error;
        toast.success("Account created! Check your inbox (and spam folder) for a confirmation email.", { duration: 6000 });
      }
    } catch (error: any) {
      const msg: string = error.message || "";
      if (msg.toLowerCase().includes("invalid login") || msg.toLowerCase().includes("invalid credentials")) {
        toast.error("Wrong email or password. Check your details or reset your password.");
      } else if (msg.toLowerCase().includes("email not confirmed")) {
        toast.error("Email not confirmed yet. Check your inbox and spam folder, or ask your admin to disable email confirmation in Supabase.", { duration: 8000 });
      } else {
          ...
          <CardContent>
            ...
            <div className="mt-4">
              ...
              <Button
                type="button"
                variant="outline"
                className="w-full rounded-full gap-2 border-primary/30 text-primary hover:bg-primary/5"
                onClick={() => {
                  enterDemo();
