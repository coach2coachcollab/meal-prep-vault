...
export function WeeklySummaryCharts() {
  ...
  const { data: chartData = [] } = useQuery({
    queryKey: queryKeys.weeklySummary(user?.id, range),
  ...
  const { data: streakData } = useQuery({
    queryKey: queryKeys.waterStreak(user?.id),
    queryFn: async () => {
      ...
      if (!data || data.length === 0) return { streak: 0, bestStreak: 0 };
      ...
      return { streak: count, bestStreak: newBest };
    },
    enabled: !!user,
  });
